...
{
    // xil_printf is lightweight; prints %s
    xil_printf("%s", s);
}
...
#endif
    n += snprintf(buf + n, sizeof(buf) - n, "[%u][%s][%s:%d %s] ", (unsigned)log_timestamp_ms(), lvlTag(lvl), file,
                  line, func);
...
void Logger::log(Level lvl, const char *file, int line, const char *func, const char *fmt, ...)
{
    ...
    va_list ap;
    va_start(ap, fmt);
    vprint(lvl, file, line, func, fmt, ap);
    va_end(ap);
}
...
{
#if LOG_LEVEL >= 5
    Logger::get().log(Logger::TRACE, file_, line_, func_, "<enter>%s", name ? name : "");
#else
    (void)name;
#endif
}
